<template>
  <div class="h-full w-full flex flex-col justify-center items-center">
    ...
    <div class="flex flex-col items-center">
      <div class="text-center text-2xl font-bold uppercase tracking-wide">Search patient by idcard</div>
      <div class="flex justify-center mt-5">
        <div class="flex text-gray-600 rounded-full border-2 border-gray-200 w-fit transition-all duration-500 items-center px-2">
          ...
          />
          <button @click="filteredUsefrs()" type="submit" class="bg-black py-2 rounded-full text-white px-5">Search</button>
        </div>
      </div>
    </div>
    ...
    <div class="flex mt-10 p-5 container gap-x-5 transition-all border" v-if="filteredUsers">
      <div class="w-1/3 h-full flex flex-col gap-y-4 px-10">
        ...
        <div class="flex justify-between items-center">
          ...
          <div class="flex w-1/2 items-center rounded-md border p-2 border-black">
            <input
              type="number"
              class="flex-grow w-1/2 focus:border-none focus:ring-0 focus:outline-none border-none"
      ...
      <div class="flex flex-col w-2/3">
        ...
        <div class="bg-gray-300 rounded-md w-full h-auto p-4 px-5 mt-5">
          <div class="w-full flex gap-x-5 items-center justify-between">
            ...
            <select class="w-full" name="" id="" v-model="medicineId">
              ...
              <option v-for="(i, index) in medicine" :key="index" :value="i.id">{{ i.name }}</option>
            </select>

            <input type="number" class="border border-black focus:ring-black focus:border-black" v-model="amount" placeholder="จำนวนเม็ด" />
            <button @click="addPill" class="bg-black text-white p-2">Add pills</button>
          </div>
          <div class="w-full mt-5 flex flex-col gap-y-2" v-for="(i, index) in medicineSelect" :key="index">
            <div class="flex justify-between items-center">
              ...
              <div class="flex items-center p-2">
                <p>{{ i.amount }} x {{ i.price }} = {{ i.amount * i.price }} บาท</p>
              </div>
            </div>
        ...
        <div class="flex justify-end py-6">
          <button @click="saveTreatment" class="bg-[#111727] border-2 boder-black text-white p-4 rounded-lg">Save All</button>
        </div>
      </div>
    </div>
  </div>
